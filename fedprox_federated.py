...
# 3. Simulate Non-IID Data (Data Heterogeneity)
...
# 6. Optimized Federated Learning (FedProx Simulation)
print("Starting Optimized Federated Learning on Non-IID Data...")
# Using SGD instead of Adam prevents momentum conflicts during manual weight updates
global_model = MLPRegressor(hidden_layer_sizes=(32, 16), max_iter=1, solver='sgd', learning_rate_init=0.01,
                            random_state=42)
...
for round_num in range(n_rounds):
    ...
    for i in range(num_clients):
        # Local Training with L2 Penalty to simulate Proximal Term (Robustness against Client Drift)
        local_model = MLPRegressor(hidden_layer_sizes=(32, 16), max_iter=10, solver='sgd',
                                   learning_rate_init=0.01, alpha=0.05, warm_start=True, random_state=42)
        local_model.fit(clients_X[i][:10], clients_y[i][:10])

        # Transfer Global Weights
...
print(f"Non-IID Federated Model (Optimized) RMSE: {rmse_fed:.4f}")
print("=" * 50)

# 7. Visualization
labels = ['Centralized Baseline\n(Unsafe)', 'Optimized Federated\n(Secure & Robust)']
rmses = [rmse_cent, rmse_fed]

plt.figure(figsize=(8, 6))
# رنگ سبز برای نشان دادن موفقیت
...
plt.tight_layout()
plt.savefig('optimized_fed_results_chart.png', dpi=300)
print("Chart successfully saved as 'optimized_fed_results_chart.png'")
plt.show()